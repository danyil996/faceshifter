...
class Trainer(nn.Module):
    def __init__(self, model_dir, g_optimizer, d_optimizer, lr, warmup, max_iters):
        ...
        self.generator, self.g_optimizer = amp.initialize(self.generator, self.g_optimizer, opt_level="O0", min_loss_scale=4, max_loss_scale=4096)
        self.discriminator, self.d_optimizer = amp.initialize(self.discriminator, self.d_optimizer, opt_level="O0", min_loss_scale=4, max_loss_scale=4096)
    ...
    def train_loop(self, dataloaders, eval_every, generate_every, save_every):
        ...
        for batch in tqdm(dataloaders['train']):
            ...
            if amp._amp_state.loss_scalers[0]._loss_scale < 8:
              amp._amp_state.loss_scalers[0]._loss = 1024
            if amp._amp_state.loss_scalers[0]._loss_scale > 4096:
                    amp._amp_state.loss_scalers[0]._loss_scale = 1024
            if torch.isnan(batch[0]).any() or torch.isnan(batch[1]).any():
                print("NaN in input data   batch number : " + str(i))
            else:
                print("data is fine : " + str(i) )
